...
int main(void)
{
  ...
  while (1)
  {
	  ...
	  if(now_time - color_cube_lasttime > color_cube_period){
		  color_cube_lasttime = now_time;
		  uint32_t cube_idx_plus1 = (cube_idx >= 2) ? (0) : (cube_idx + 1);

		  if(colors[cube_idx] > 0) {
			  colors[cube_idx]--;
			  colors[cube_idx_plus1]++;
		  } else{
			  cube_idx = cube_idx_plus1;
		  }
...
static void MX_GPIO_Init(void)
{
  ...
  HAL_GPIO_WritePin(GPIOA, MD_AIN2_Pin|MD_AIN1_Pin|MD_STBY_Pin|MD_BIN1_Pin
                          |MD_BIN2_Pin, GPIO_PIN_RESET);
  ...
  /*Configure GPIO pins : MD_AIN2_Pin MD_AIN1_Pin MD_STBY_Pin MD_BIN1_Pin
                           MD_BIN2_Pin */
  GPIO_InitStruct.Pin = MD_AIN2_Pin|MD_AIN1_Pin|MD_STBY_Pin|MD_BIN1_Pin
                          |MD_BIN2_Pin;
...
void HAL_TIM_IC_CaptureCallback(TIM_HandleTypeDef* htim){
	if(htim->Instance == htim4.Instance){ // Rotary encoder
		uint32_t counter = __HAL_TIM_GET_COUNTER(htim);
		rot_enc_pos = (int32_t)counter;
		...
		uint16_t dist = ((uint16_t)HAL_TIM_ReadCapturedValue(htim, TIM_CHANNEL_2)) - ((uint16_t)HAL_TIM_ReadCapturedValue(htim, TIM_CHANNEL_1));

		if(dist < MT6701_MIN_PWM) return; // Invalid
